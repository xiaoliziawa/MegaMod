...
//
//         [Tooltip("Strength of the sharpening effect.")] [Range(0, 1)]
//         public float sharpness = 0.8f;
//
//         [Tooltip("Adjust the influence of motion vectors on temporal accumulation.")] [Range(0, 1)]
//         public float velocityFactor = 1.0f;
//
//         [Header("Exposure")]
//         [Tooltip(
//             "Allow an exposure value to be computed internally. When set to false, either the provided exposure texture or a default exposure value will be used.")]
//         public bool enableAutoExposure = true;
//
//         [Tooltip(
//             "Value by which the input signal will be divided, to get back to the original signal produced by the game.")]
...
//
//         [Header("Debug")] [Tooltip("Enable a debug view to analyze the upscaling process.")]
//         public bool enableDebugView = false;
//
//         [Header("Reactivity, Transparency & Composition")]
//         [Tooltip(
//             "Optional texture to control the influence of the current frame on the reconstructed output. If unset, either an auto-generated or a default cleared reactive mask will be used.")]
//         public Texture reactiveMask = null;
//
//         [Tooltip(
//             "Optional texture for marking areas of specialist rendering which should be accounted for during the upscaling process. If unset, a default cleared mask will be used.")]
//         public Texture transparencyAndCompositionMask = null;
//
//         [Tooltip(
//             "Automatically generate a reactive mask based on the difference between opaque-only render output and the final render output including alpha transparencies.")]
//         public bool autoGenerateReactiveMask = true;
//
//         [Tooltip("Parameters to control the process of auto-generating a reactive mask.")] [SerializeField]
//         private GenerateReactiveParameters generateReactiveParameters = new GenerateReactiveParameters();
//
...
//         {
//             [Tooltip("A value to scale the output")] [Range(0, 2)]
//             public float scale = 0.5f;
//
//             [Tooltip("A threshold value to generate a binary reactive mask")] [Range(0, 1)]
//             public float cutoffThreshold = 0.2f;
//
//             [Tooltip("A value to set for the binary reactive mask")] [Range(0, 1)]
...
//
//         [Tooltip(
//             "(Experimental) Automatically generate and use Reactive mask and Transparency & composition mask internally.")]
...
//         [SerializeField]
//         private GenerateTcrParameters generateTransparencyAndCompositionParameters = new GenerateTcrParameters();
//
...
//
//             [Tooltip("Smaller values will increase stability at hard edges of translucent objects.")] [Range(0, 2)]
//             public float autoTcScale = 1.0f;
//
//             [Tooltip("Larger values result in more reactive pixels.")] [Range(0, 10)]
//             public float autoReactiveScale = 5.0f;
//
//             [Tooltip("Maximum value reactivity can reach.")] [Range(0, 1)]
//             public float autoReactiveMax = 0.9f;
//         }
//
//         [SerializeField] public Fsr3UpscalerAssets assets;
//
...
//
//         private readonly Fsr3Upscaler.GenerateReactiveDescription _genReactiveDescription =
//             new Fsr3Upscaler.GenerateReactiveDescription();
//
...
//             _renderCamera.targetTexture =
//                 null; // Clear the camera's target texture so we can fully control how the output gets written
//             _renderCamera.depthTextureMode =
//                 _originalDepthTextureMode | DepthTextureMode.Depth | DepthTextureMode.MotionVectors;
//
//             // Determine the desired rendering and display resolutions
//             _displaySize = GetDisplaySize();
//             Fsr3Upscaler.GetRenderResolutionFromQualityMode(out var maxRenderWidth, out var maxRenderHeight,
//                 _displaySize.x, _displaySize.y, qualityMode);
...
//             {
//                 Debug.LogError(
//                     $"FSR3 Upscaler assets are not assigned! Please ensure an {nameof(Fsr3UpscalerAssets)} asset is assigned to the Assets property of this component.");
//                 enabled = false;
//                 return;
//             }
//
...
//             {
//                 Debug.LogError(
//                     $"FSR3 Upscaler render size is invalid: {_maxRenderSize.x}x{_maxRenderSize.y}. Please check your screen resolution and camera viewport parameters.");
//                 enabled = false;
//                 return;
//             }
//
...
//                 _renderCamera.rect = new Rect(0, 0, _originalRect.width * _maxRenderSize.x / _renderCamera.pixelWidth,
...
//                 _colorOpaqueOnly =
//                     RenderTexture.GetTemporary(scaledRenderSize.x, scaledRenderSize.y, 0, GetDefaultFormat());
...
//                 new ResourceView(BuiltinRenderTextureType.CameraTarget, RenderTextureSubElement.Color);
...
//                 (_dispatchDescription.CameraNear, _dispatchDescription.CameraFar) = (_dispatchDescription.CameraFar,
//                     _dispatchDescription.CameraNear);
//             }
//         }
//
...
//             _genReactiveDescription.ColorPreUpscale =
//                 new ResourceView(BuiltinRenderTextureType.CameraTarget, RenderTextureSubElement.Color);
...
//                 var scaledRenderSize = GetScaledRenderSize();
//                 _dispatchCommandBuffer.GetTemporaryRT(Fsr3ShaderIDs.UavAutoReactive, scaledRenderSize.x,
//                     scaledRenderSize.y, 0, default, GraphicsFormat.R8_UNorm, 1, true);
...
//             _dispatchCommandBuffer.GetTemporaryRT(Fsr3ShaderIDs.UavUpscaledOutput, _displaySize.x, _displaySize.y, 0,
//                 default, GetDefaultFormat(), default, 1, true);
//
...
//                 _dispatchCommandBuffer.Blit(Fsr3ShaderIDs.UavUpscaledOutput, _originalRenderTarget,
//                     _copyWithDepthMaterial);
//             }
//             else
//             {
...
//         {
//             return _renderCamera.allowDynamicResolution ||
//                    (_originalRenderTarget != null && _originalRenderTarget.useDynamicScale);
//         }
//
//         private Vector2Int GetScaledRenderSize()
//         {
//             if (UsingDynamicResolution())
//                 return new Vector2Int(Mathf.CeilToInt(_maxRenderSize.x * ScalableBufferManager.widthScaleFactor),
//                     Mathf.CeilToInt(_maxRenderSize.y * ScalableBufferManager.heightScaleFactor));
//
